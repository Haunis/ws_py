...
DataFrame.fillna(value=None, method=None, axis=None, inplace=False, limit=None)
    ...
    method:	插值方式
            ffill--使用na前面的列的值进行填充
            bfill--使用na后面的列的值进行填充
    axis:	待填充的轴，默认axis=0
...
df.loc[:3, 1] = NA  # [0,3]行,第1列
df.loc[:2, 2] = NA  # [0,2]行,第2列
print(df, end="\n\n")
ret = df.fillna({1: 1.111, 2: 2.222})
print("填充后：\n%s"%ret.__str__())
...
# print(df, end="\n\n")
ret = df.fillna(method='bfill')  # 就是用该列的前一个值填充
print(ret)
...
print(df, end="\n\n")
df[1] = df[1].fillna(df[1].mean()) # df[1]--选取column名为1的列
print(df)
